...
RSpec.describe ItemCategory, type: :model do
  ...
  it 'is valid with a item_id and a category_id' do
    item_category = FactoryBot.build(:item_category)
    expect(item_category).to be_valid
  end

  it 'is invalid without a item_id' do
    item_category = FactoryBot.build(:item_category, item_id: nil)
    item_category.valid?
    expect(item_category.errors[:item_id]).to include("can't be blank")
  end
  ...
  it 'is invalid with a duplicate item_id and category_id' do
    item_category1 = FactoryBot.create(:item_category, item_id: 1, category_id: 1)
    item_category2 = FactoryBot.build(:item_category, item_id: 1, category_id: 1)
